...
# THIS IS NOW DEPRECATED FOR ALL WEBSITE USES. ONLY USE FOR STATISTICS
...
class Cache:
    ...
    def create_cache(self) -> None:
        movies: PostgrestAPIResponse[Movie] = (
            supabase.table("movie").select("id", "numbers_slug").execute()
        )

        numbers_slug_to_db_id: dict[str, str] = {}

        for movie in movies.data:
            numbers_slug_to_db_id[movie["numbers_slug"]] = movie["id"]

        self.movie_cache = numbers_slug_to_db_id

        print(
            f"{bcolors.OKGREEN}Created movie cache with {len(numbers_slug_to_db_id)} entries{bcolors.ENDC}"
        )

        genres: PostgrestAPIResponse[Genre] = (
            supabase.table("genre").select("id", "tmdb_id").execute()
        )

        genre_tmdb_id_to_db_id: dict[int, str] = {}

        for genre in genres.data:
            genre_tmdb_id_to_db_id[genre["tmdb_id"]] = genre["id"]

        self.genre_cache = genre_tmdb_id_to_db_id

        print(
            f"{bcolors.OKGREEN}Created genre cache with {len(genre_tmdb_id_to_db_id)} entries{bcolors.ENDC}"
        )

        collections: PostgrestAPIResponse[Collection] = (
            supabase.table("collection").select("id", "tmdb_id").execute()
        )

        collection_tmdb_id_to_db_id: dict[int, str] = {}

        for collection in collections.data:
            collection_tmdb_id_to_db_id[collection["tmdb_id"]] = collection["id"]

        self.collection_cache = collection_tmdb_id_to_db_id

        print(
            f"{bcolors.OKGREEN}Created collection cache with {len(collection_tmdb_id_to_db_id)} entries{bcolors.ENDC}"
        )

        production_companies: PostgrestAPIResponse[ProductionCompany] = (
            supabase.table("production_company").select("id", "tmdb_id").execute()
        )

        production_company_tmdb_id_to_db_id: dict[int, str] = {}

        for production_company in production_companies.data:
            production_company_tmdb_id_to_db_id[production_company["tmdb_id"]] = (
                production_company["id"]
            )

        self.production_company_cache = production_company_tmdb_id_to_db_id

        print(
            f"{bcolors.OKGREEN}Created production company cache with {len(production_company_tmdb_id_to_db_id)} entries{bcolors.ENDC}"
        )

        production_countries: PostgrestAPIResponse[ProductionCountry] = (
            supabase.table("production_country").select("id", "iso_3166_1").execute()
        )

        production_country_iso_to_db_id: dict[str, str] = {}

        for production_country in production_countries.data:
            production_country_iso_to_db_id[production_country["iso_3166_1"]] = (
                production_country["id"]
            )

        self.production_country_cache = production_country_iso_to_db_id

        print(
            f"{bcolors.OKGREEN}Created production country cache with {len(production_country_iso_to_db_id)} entries{bcolors.ENDC}"
        )

        spoken_languages: PostgrestAPIResponse[SpokenLanguage] = (
            supabase.table("spoken_language").select("id", "iso_639_1").execute()
        )

        spoken_language_iso_to_db_id: dict[str, str] = {}

        for spoken_language in spoken_languages.data:
            spoken_language_iso_to_db_id[spoken_language["iso_639_1"]] = (
                spoken_language["id"]
            )

        self.spoken_language_cache = spoken_language_iso_to_db_id

        print(
            f"{bcolors.OKGREEN}Created spoken language cache with {len(spoken_language_iso_to_db_id)} entries{bcolors.ENDC}"
        )

    def get_movie_id(self, numbers_slug: str) -> str | None:
        if numbers_slug not in self.movie_cache:
            return None
    ...
    def get_or_create_genre(self, name: str, tmdb_id: int) -> str:
        ...
        if genre_id is None:
            genre: PostgrestAPIResponse[Genre] = (
    ...
    def get_or_create_production_company(
        ...
        if production_company_id is None:
            production_company: PostgrestAPIResponse[ProductionCompany] = (
    ...
    def get_or_create_production_country(self, name: str, iso_3166_1: str) -> str:
        ...
        if production_country_id is None:
            production_country: PostgrestAPIResponse[ProductionCountry] = (
    ...
    def get_or_create_spoken_language(self, name: str, iso_639_1: str) -> str:
        ...
        if spoken_language_id is None:
            ...
            spoken_language: PostgrestAPIResponse[SpokenLanguage] = (
    ...
    def get_or_create_movie(self, movie_to_add: AddableMovie) -> Movie:
        ...
        if movie_id is None:
            movie: PostgrestAPIResponse[Movie] = (
    ...
    def create_or_update_numbers_daily(
        ...
        daily: PostgrestAPIResponse[Boxofficeday] = (
    ...
    def get_or_create_movie_info(
        ...
        movie_info: PostgrestAPIResponse[MovieInfoDay] = (
    ...
    def bulk_create_box_office_days(
        ...
        existing_box_office_days: PostgrestAPIResponse[Boxofficeday] = (
            supabase.table("boxofficeday")
        ...
        if len(box_office_dumps) == 0:
            ...
            response: PostgrestAPIResponse[Boxofficeday] = (
                supabase.table("boxofficeday").insert(box_office_dumps).execute()
            )
    ...
    def bulk_create_release_dates(
        ...
        existing_release_dates: PostgrestAPIResponse[MovieReleaseDate] = (
